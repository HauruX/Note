...
使函数中的变量一直保留在内存中。被嵌套的函数的引用不被重置为`null`的话，闭包环境就不会被垃圾回收机制释放掉。因此滥用闭包回导致效率问题，在IE中甚至导致内存泄漏。
...
闭包内部函数的`this`一般指向全局变量`window`，可以通过外层函数设置`var _this = this`解决。
...
基于类的语言中，继承提供两个有用的服务，一是代码复用的一种形式（当一个新的类和一个已存在的类大部分相似时，复用可以显著减少软件开发成本），二是引入一套类型系统规范（程序员无需编写显式类型转换，从而减少工作量，但会带来安全风险）。

javascript是弱类型语言，对象继承关系变得无关紧要，对于对象来说重要的是它能做什么，而不是它从哪里来。
